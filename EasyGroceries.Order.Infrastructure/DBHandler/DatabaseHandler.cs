...
namespace EasyGroceries.Order.Infrastructure.DBHandler
{
    public class DatabaseHandler : IDapper
    {
        ...
        public int InsertList<T>(string sp, T data, CommandType commandType = CommandType.StoredProcedure)
        {
            int result;
            using IDbConnection db = new SqlConnection(_connectionString);
            try
            {
                ...
                try
                {
                    result = db.Execute(sp, data, commandType: commandType, transaction: tran);
                    tran.Commit();
                }
                catch (Exception ex)
                {
